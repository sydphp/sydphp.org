...
class ImageGalleryPage extends Page
{
	...
	*/
	...
	public function getCMSFields($cms) {
		...
		$f->addFieldToTab("Root.Content.$configuration", new NumericField('NormalSize',_t('ImageGalleryPage.NORMALSIZE','Normal size (pixels)')));
		...
		$f->addFieldToTab("Root.Content.$configuration", new NumericField('UploadLimit',_t('ImageGalleryPage.MAXFILES','Max files allowed in upload quue')));				
	...
	public function CurrentAlbum()
	{
		...
		if(isset($c->urlParams['ID'])) {
			$url_segment = $c->urlParams['ID'];
			$albums = DataObject::get($this->albumClass,"ImageGalleryPageID = {$this->ID} AND File.Name = '$url_segment'", "", "LEFT JOIN File ON File.ID = FolderID");
	...
	//CODEM - check for the request variable
	public function NextGalleryItems()
	{
		if(isset($_REQUEST['start']) && $_REQUEST['start'] > 0 && $this->MediaPerPage)
